...
async def modhelp(ctx):
    ...
    embed.set_image(url = 'https://image.ibb.co/caM2BK/help.gif')
    embed.add_field(name = '/say(Admin permission required) ',value ='Use it like ``d!say <text>``',inline = False)
    ...
    embed.add_field(name = '/kick(Kick members Permission Required)',value ='Use it like ``d!kick @user`` to kick any user',inline = False)
    ...
    embed.add_field(name = '/getuser(Kick members Permission Required) ',value ='Use it like ``d!getuser @rolename`` to get list of all users having a particular role',inline = False)
    await client.send_message(author,embed=embed)
    await client.say('📨 Check DMs For Information')
